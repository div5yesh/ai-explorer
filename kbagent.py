...
class KBAgentRogue(BaseAgent):
    ...
    def evaluate_agent_exploration(self, map):
    ...
    def step(self, location, strength, game_map, map_objects):
        ...
        if len(actions) == 0 and \
                self.kb.has_enough_strength_for_skeleton(strength) and \
                self.kb.has_enough_strength_for_dynamic_monster(strength):
...
def plan(start, goals, problem, states):
    best_decision = tuple(([], sys.maxsize))
    if len(goals):
        for goal in goals:
            if goal:
                node_result = AStarSearch(start, goal, problem, states)
                if best_decision[1] > node_result[1]:
                    best_decision = node_result
